...
void CEulerSolver::SetNondimensionalization(CConfig *config, unsigned short iMesh) {
  ...
  SU2_OMP_PARALLEL
  {
    ...
    switch (config->GetKind_FluidModel()) {
      ...
      case TABLE_FLUID:
        FluidModel[thread] = new CTableFluid();
        break;
    }
...
void CEulerSolver::Evaluate_ObjFunc(CConfig *config) {
  ...
  switch(Kind_ObjFunc) {
    ...
    case TOTAL_AVG_TEMPERATURE:
      ...
      break;
    case CUSTOM_OBJFUNC:
      Total_ComboObj+=Weight_ObjFunc*config->GetSurface_MassFlow(1)/config->GetSurface_MassFlow(0);
      break;
    default:
      break;
  }
